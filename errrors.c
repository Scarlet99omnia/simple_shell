...
/**
 * errconvstring - converts a string to an integer
 * @s: the string to be converted
 * Return: 0 if no numbers in string, converted number otherwise
 *       -1 on error
 */
int errconvstring(char *s)
{
	int i = 0;
 ...
 *        -1 on error
 */
void printError(inft *info, char *estring)
{
	printsinput(info->filename);
	printsinput(": ");
	printdec(info->errcount, STDERR_FILENO);
	printsinput(": ");
	printsinput(info->argv[0]);
	printsinput(": ");
	printsinput(estring);
}
...
/**
 * rmvComments - function replaces first instance of '#' with '\0'
